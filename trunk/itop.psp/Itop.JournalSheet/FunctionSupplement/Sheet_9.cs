...
/******************************************************************************************************
 *  ClassName：Sheet_9
 *  Action：附表34  XX市高压配电网主变容量需求分析 单位：MW、MVA（发策部、生技部、四县公司）表的数据写入
 * Author：吕静涛
 * Mender  ：吕静涛
...
namespace Itop.JournalSheet.FunctionSupplement
{
    class Sheet_9
    {
        ...
        private const int intProject=5;
        ...
        public void SetSheet_9Title(Itop.Client.Base.FormBase FB, FarPoint.Win.Spread.SheetView obj, string Title,bool IsTrue)
        {
            ...
            strProject[1] = "容载比选取";
            strProject[2] = "主变容量需求";
            strProject[3] = "2009年底主变容量";
            strProject[4] = "需新增主变容量";
        ...
        private void SetLeftTitle(FarPoint.Win.Spread.SheetView obj, int IntRow, int IntCol)
        {
            ...
            list = AreaName[0];
            for (int i = 0; i < AreaName[0].Count * (VoltileClass * intProject); i += 10)
            {
                PF.CreateSheetView(obj, (VoltileClass * intProject), 1, IntRow + i, 1, list[index].ToString());
                intdexVC = 0;
                for (int j = 0; j < VoltileClass*intProject; j+=5)
                {
            ...
            index = 0;
            intdexVC = 0;
            for (int i = 0; i < AreaName[1].Count * (VoltileClass * intProject); i += 10)
            {
        ...
        {
            for (int i = IntRow; i < obj.RowCount; i += 10)//行从第IntRow行开始载入
            {
                for (int l = 0; l < SaveList.Count; ++l)
                {
                    if ((SaveList[l].strType == PF.ReturnStr(obj, i, 0).ToString()) && (SaveList[l].strDQ == PF.ReturnStr(obj, i, 1).ToString()))
                    {
                        for (int j = IntCol; j < obj.ColumnCount; ++j)//
                        {
                            ...
                            obj.SetValue(i + 6, j, SaveList[l].data[6, j - IntCol]);
                            obj.SetValue(i + 7, j, SaveList[l].data[7, j - IntCol]);
                            obj.SetValue(i + 8, j, SaveList[l].data[8, j - IntCol]);
                            obj.SetValue(i + 9, j, SaveList[l].data[9, j - IntCol]);
